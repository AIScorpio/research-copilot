        ...
        switch (action) {
            case 'test': {
                ...
                return NextResponse.json({
                    ...
                    availableModels: isConnected ? availableModels : []
                });
            }
...
async function fetchAvailableModels(config: any): Promise<string[]> {
    // Return models from base that are available for this provider
    ...
    return models.map(m => m.externalId);
}
